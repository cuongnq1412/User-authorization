...
class DistrictModel extends BaseModel
{
    ...
    public function acceptGroup($id)
    {
        ...
        $query1 = "UPDATE profile SET check_approved = 3 WHERE association_id = ?";
        $query2 = "UPDATE association SET status = 3 WHERE id = ?";
    ...
    public function refuseGroup($id)
    {
        ...
        // Sử dụng chuẩn bị trước (prepared statements) để cập nhật dữ liệu
        $query1 = "UPDATE profile SET check_approved = 2 WHERE association_id = ?";
        $query2 = "UPDATE association SET status = 2 WHERE id = ?";
    ...
    public function detailaceptGroup($id)
    {
        ...
        WHERE profile.association_id = '$id' AND profile.check_approved >= 3";
        $result = $this->db->conn->query($query);
        $data = array();
        while ($row = $result->fetch_assoc()) {
            $data[] = $row;
